...
class TestViews(TestBase):
    ...
    def test_dashboard_view(self):
        ...
        """
        target_url = url_for('hrequests.index')
        redirect_url = url_for('auth.login', next=target_url)
        response = self.client.get(target_url)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, redirect_url)
    ...
    def test_clients_view(self):
        """
        Test that clients page is inaccessible without login
        and redirects to login page then to departments page
        """
        target_url = url_for('clients.index')
        redirect_url = url_for('auth.login', next=target_url)
        response = self.client.get(target_url)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, redirect_url)
    ...
    def test_admin_users_view(self):
        ...
        """
        target_url = url_for('admin.users_index')
        redirect_url = url_for('auth.login', next=target_url)
        response = self.client.get(target_url)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, redirect_url)
